...
fn split_files(records: Vec<StringRecord>) -> Result<Vec<StringRecord>> {
    let fist_store = dbg!(records[0].get(0).unwrap().to_string());
    let mut new_list: Vec<StringRecord> = Vec::new();
...
fn main() -> Result<()> {
    ...
    let results = split_files(results)?;
    write_file(results);
    Ok(())
}
